...
of `interactions`/`impressions` are excluded by name pattern (`interactions_pYYYY_MM_DD / impressions_pYYYY_MM_DD`) — they
...
Deliberately independent of `now_platform_db.schema_hash` (see `partitions.py` for
...
_PARTITION_RE = re.compile(r"^(interactions|impressions)_p\d{4}_\d{2}_\d{2}$")
