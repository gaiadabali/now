...
from now_db.settings import explicit_city_database_url
...
# No ORM metadata object: migrations are hand-written SQL. This same script
# runs, unmodified, against every city DB — see now_db.provisioning.
target_metadata = None


def get_url() -> str:
    return config.get_main_option("sqlalchemy.url") or explicit_city_database_url() or ""
...
def run_migrations_online() -> None:
    ...
    if not url:
        raise RuntimeError(
            "no target database URL: pass --url to `now-db migrate`, set "
            "NOW_CITY_DATABASE_URL, or set sqlalchemy.url in alembic.ini"
        )
