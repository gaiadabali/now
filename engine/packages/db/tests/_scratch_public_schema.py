...
`now_db.facet_sync`'s self-test (F132) needs, against `now_test`. Copied
from `now_eval`'s `tests/calibration/_scratch_public_schema.py` (same
need, same precedent: `apply_llm_labels.py`'s own integration tests)
rather than shared across packages, matching that module's own "this is
scaffolding, not a migration" framing.
